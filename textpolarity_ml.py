# -*- coding: utf-8 -*-
"""
Created on Mon Oct 23 15:16:04 2017
...
def clean_string_to_process(string):
    """
...
def load_data_to_classify():
    """
    Splits sentences and returns the splits and the target labels
    """
    # Pull sentences with positive sentiment
    pos_file = open('rt-polarity.pos', mode='read')
    # Pull sentences with negative sentiment
    neg_file = open('rt-polarity.neg', mode='read')


    # Load data from files
    pos_ex = list(pos_file.readlines())
    pos_ex = [s.strip() for s in pos_ex]
    neg_ex = list(neg_file.readlines())
    neg_ex = [s.strip() for s in neg_ex]
    # Split by words
    x_text = pos_ex + neg_ex
    x_text = [clean_string_to_process(sent) for sent in x_text]
    x_text = [s.split(" ") for s in x_text]
    # Generate labels
    pos_labels = [1 for _ in pos_ex]
    neg_labels = [0 for _ in neg_ex]
    y = np.concatenate([pos_labels, neg_labels], 0)
    return [x_text, y]


def zero_padding(sentences, padding_word=""):
    """
...
def building_ip_data_tomodel(sentences, labels, vocabulary):
    """
...
sentences, labels = load_data_to_classify()
sentences_padded = zero_padding(sentences)
vocabulary, vocabulary_inv = vocab(sentences_padded)
x, y = building_ip_data_tomodel(sentences_padded, labels, vocabulary)
